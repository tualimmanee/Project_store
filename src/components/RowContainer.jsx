  ...
  >
    {data && data.length > 0 ? (
      data.map((item) => (
        <div
          key={item?.id}
          className="w-275 h-[175px] min-w-[275px] md:w-300 md:min-w-[300px] drop-shadow-2xl bg-cardOverlay rounded-lg py-2 px-4  my-12 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-evenly relative"
        >
            ...
            <div className="flex flex-col  gap-2 w-130">
              ...
              <div className="flex flex-row justify-between">
                {user && user.email === "osuzumiyao@gmail.com" && (
                  <div>
                    <motion.div className="w-9 h-5 text-white rounded-full bg-red-500 flex flex-row items-center justify-center cursor-pointer hover:shadow-md "
                      whileTap={{ scale: 0.75 }}
                      onClick={() => seteditbox(true)}>edit</motion.div>
